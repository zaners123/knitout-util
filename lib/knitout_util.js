...
function* range(x1,x2, dir,inc=1) {
	if (dir==='+') {
		for (let i=x1;i<=x2;i+=inc) yield i;
	} else {
		for (let i=x2;i>=x1;i-=inc) yield i;
	}
}
...
class KnitoutUtil extends knitout.Writer {
	...
	constructor() {
		...
		this.STOCKINETTE_FG = {//Note: This isn't half-gauge
	...
	static shape_round_to_gauge(shape, gauge) {
		for (let i=0;i<shape.length;i++) shape[i]=shape[i] - shape[i]%gauge
		return shape
	}

	/**Makes a patterned array to use in knits
	 * @param safe_ends Make the ends round off (res[0]===res[1] and res[N-2]===res[N-1])
	 * */
	static shape_line(courses, x1a, x1b, safe_ends = false) {
		...
		if (safe_ends) {
			return [res[0]].concat(res).concat(res[res.length-1])
		} else {
			return res
		}
	}
	...
	/**Bind-on a square or closed tube using a pattern
	 ...
	 * */
	gen_bindon_fs(pattern, carrier, X1, X2) {
		return function* g(k) {
	...
	bindoff_needle(carrier, bed, wale, dir) {
		this.knit(dir, bed + wale, carrier);
		this.miss(dir, bed + (wale + ((dir==="+")?1:-1)), carrier);//pull
	}

	/**
	 * At wale loc, make a tieoff tail/tag like a left flag that could be knitted back in
	 * */
	bindoff_tail(carrier, wale, hooksTagged=1) {
		for (let size of [
			hooksTagged,    hooksTagged,	hooksTagged,
			hooksTagged+1,	hooksTagged+1,
			hooksTagged+2,  hooksTagged+2,
			hooksTagged+3,	hooksTagged+3,
		]) {
			let x1 = wale-size
			let x2 = wale
			this.knit_line_of_tube(['k'],carrier, x1, wale, '-','f')
			this.miss("-", "f"+(x1-1), carrier);
			this.knit_line_of_tube(['k'],carrier, x1, wale, '+','f')
			this.miss("+", "f"+(x2+1), carrier);
		}
		for (let i=0;i<2;i++) for (let x = wale-6; x<wale+6;x++) this.drop('f'+x)

	}

	/**
	 * Bindoff a halfgauge tube in a way that leaves it open, such as a straw
	 * @deprecated TODO does not work
	 * */
	gen_bindoff_hg_opentube(carrier, x1, x2) {
		if (x2%2!==0) x2++
		return function* g(k) {
			//bindoff front
			let wale = x1
			for (;wale < x2; wale += 2) {
				...
				k.rack(1);
				k.xfer("b"+(wale+1), "f"+(wale+2));//transfer right
				k.bindoff_needle(carrier,'f',wale+2,'+')
			}

			//move front bindoff to back
			k.rack(1);
			k.xfer('f' + (wale), 'b' + (wale-2));
	 ...
	 * */
	gen_bindoff_fg_closed(carrier, x1, x2) {
		let hooksTagged = 3
		return function* g(k) {
			for (let wale = x1; wale <= x2; wale++) {
				k.xfer("b"+wale,"f"+wale)
			}
			for (let i=0;i<2;i++) {
				k.knit_line_of_tube(k.STOCKINETTE_FG,carrier,x1,x2,'+','f')
				k.knit_line_of_tube(k.STOCKINETTE_FG,carrier,x1,x2,'-','f')
			}

			for (let wale = x1; wale <= x2-hooksTagged+1; wale++) {
			...
			k.bindoff_tail(carrier, x2,hooksTagged);
			yield {'dir': '+', 'wale': x2};
		}(this);
	}
	 ...
	 * @param x1Arr leftside  wale to subtract successive diam_generator calls from
	 * @param x2Arr rightside wale to add      successive diam_generator calls from
	 * @param starting_course
	 * */
	gen_arbtube_varying_sides(pattern, carrier, x1Arr, x2Arr, starting_course=0) {
		if (x1Arr.length !== x2Arr.length) throw "Different lengths"
		return function* g(k) {
			let x1Last = x1Arr[0]
			let x2Last = x2Arr[0]
			...
			let inc = 0;
			if (pattern.gauge==='full') inc=1
			else if (pattern.gauge==='half') inc=2
			else throw "Unknown inc"

			for (let index = 0;index < x1Arr.length; index++) {
				let x1 = x1Arr[index]
				let x2 = x2Arr[index]

				if (x1%inc !== 0 || x2%inc !== 0) throw "x1 and x2 must be divisible by gauge:\n"+
					"\tx1Arr["+index+"]="+x1+"\n\tx2Arr["+index+"]="+x2

				if (x1 < x1Last) {
					k.comment("Grow X1 by "+(x1Last-x1))
					for (let wale of range(x1+inc,x1Last,'-',inc)) {
						k.rack(inc)
						k.split('-','f'+wale,         'b'+(wale-inc),carrier)
						k.rack(0)
						k.split('-','b'+(wale-inc),   'f'+(wale-inc),carrier)
						k.knit( '+','f'+(wale-inc),                  carrier)
					}
				} else if (x1 > x1Last) {
					k.comment("Shrink X1 by "+(x1-x1Last))
					for (let wale of range(x1Last,x1-inc,'+',inc)) {
						k.xfer("f"+wale,"b"+wale);
						k.rack(inc);
						k.xfer("b"+wale,"f"+(wale+inc  ));
						k.knit("+",     "f"+(wale+inc  ), carrier);
						k.miss("+",     "f"+(wale+inc+1), carrier);
						k.rack(0);
					}
				}
				k.knit_line_of_tube(pattern.front[course%pattern.front.length], carrier, x1, x2, '+','f')
				yield {"dir":'+',"loc":x2}

				//main same for X2
				if (x2 > x2Last) {
					k.comment("Grow X2 by "+(x2-x2Last))
					for (let wale of range(x2Last,x2-inc,'+',inc)) {
						k.rack(-inc)
						k.split('+','f'+wale,       'b'+(wale+inc),carrier)
						k.rack(0)
						k.split('+','b'+(wale+inc), 'f'+(wale+inc),carrier)
					...
					k.comment("Shrink X2 by "+(x2Last-x2))
					for (let wale of range(x2+inc,x2Last,'-',inc)) {
						k.xfer("f"+wale,"b"+wale);
						k.rack(-inc);
						k.xfer("b"+wale,"f"+(wale-inc));
						k.knit("-", "f"+(wale-inc  ), carrier);
				...
				k.knit_line_of_tube(pattern.back[course%pattern.back.length], carrier, x1, x2, '-','b')
				yield {"dir":'-',"loc":x1}
	...
	/** Generate on only one side a
	 * NOTE: Uses tubeknit, so transfers more than necessary if not called on both sides of bed.
	 * @param pattern A knit pattern, such as this.STOCKINETTETUBE
	 * @param carrier The yarn carrier
	 * @param x1Arr leftside  wale to subtract successive diam_generator calls from
	 * @param x2Arr rightside wale to add      successive diam_generator calls from
	 * @param starting_course The course started on in the pattern
	 * @param {string} bed
	 * */
	gen_arbsquare_varying_sides(pattern, carrier, x1Arr, x2Arr, starting_course=0, bed='f') {
		if (x1Arr.length !== x2Arr.length) throw "Different lengths"
		if (pattern.gauge !== "half") throw "Not implemented"
		return function* g(k) {
			let x1Last = x1Arr[0]
			let x2Last = x2Arr[0]
			let course = starting_course
			for (let i = 0; i < x1Arr.length; i++) {
				...
				k.knit_line_of_tube(bed==='f'?pattern.front[course%pattern.front.length]:pattern.back[course%pattern.back.length], carrier, x1, x2, '+', bed)
				yield;
