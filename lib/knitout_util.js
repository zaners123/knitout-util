...
class KnitoutUtil extends knitout.Writer {
	...
	constructor() {
		...
		this.COMPACT_STOCKINETTE = {//Note: This isn't half-gauge
	...
	gen_bindon_tube_open(pattern, carrier, X1, X2) {
		return function* g(k) {
			...
			k.run(k.gen_arbtube(pattern, carrier, 1, X1, X2))
		}(this);
	}
	...
	bindoff_tail(carrier, wale, facingRight=true) {
		for (let x of [0,0,0,1,2,3,4,4,4,4]) {
	...
	gen_bindoff_closed(carrier, x1, x2, facingRight = undefined) {
		return function* g(k) {
			...
			k.xfer("b"+x2,"f"+x2)
			k.bindoff_tail(carrier, x2,facingRight);
			yield {'dir': '+', 'wale': x2};
		}(this);
	}
